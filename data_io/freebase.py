...
def main_api():
    ...
    col = []
    type_list = []
    with open("data/raw/Freebase/link.dat", "r") as f:
    ...
    with open("data/raw/Freebase/node.dat", "r") as f:
        for line_id, x in enumerate(f):
            y = x.split('\t')
            node_type = int(y[2])
...
def main_api_type():
    ...
    # ratios = (0.9, 0.04, 0.06, 0.1)
    # ratios = (0.8, 0.09, 0.11, 0.1)
    # ratios = (0.7, 0.146, 0.154, 0.1)
    # ratios = (0.6, 0.195, 0.205, 0.1)
    ratios = (0.5, 0.247, 0.253, 0.1)
    # ratios = (0.4, 0.3, 0.3, 0.1)
    if ratios[0] < 1:
