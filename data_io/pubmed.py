...
def main_api():
    ...
    col = []
    type_list = []
    with open("data/raw/PubMed/link.dat", "r") as f:
    ...
    with open("data/raw/PubMed/node.dat", "r") as f:
        for line_id, x in enumerate(f):
            y = x.split('\t')
            node_type = int(y[2])
...
def main_api_type():
    ...
    values_t = attributes_full[lying_t] + np.random.normal(0, 0.01, [len(node_types_t), d0])
